<?php
	...
	$FILTRO = "WHERE TB1.PRODUCTO LIKE '%".$_GET["BUSCAR"] ."%'";
	
	if($_GET["LOTE"]){
		$LOTE = "AND TB2.LOTE='".$_GET["LOTE"]."'";
	}
...
//Declaramos las variables
	...
	if ($result->fetchColumn() <= 0) {
		print '<script>alertify.alert("El producto no se encuentra en el almacen");</script>';
		...
		foreach ($conn->query($sql) as $row) {
			print "<script>	$('.app-title').html('<a style=\'font-size:13px !important;\'>".RecortarDescripcion($row["PRODUCTO"]." - ".$row["DESCRIPCION"])."'); </script>";
